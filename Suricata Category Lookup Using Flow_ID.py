...
def run():
    ...
    for honeypot, url in HONEYPOTS.items():
        ...
        category_total = sum(bucket["flow_count"]["value"] for bucket in category_response["aggregations"]["signatures"]["buckets"])
        ...
        total_count = sum(bucket["flow_count"]["value"] for bucket in total_response["aggregations"]["signatures"]["buckets"])
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for honeypot in HONEYPOTS:
            ...
            f.write(f"Total alerts in category (unique flow_id): {cat_total}\n")
            f.write(f"Total alerts overall (unique flow_id):     {total}\n")
